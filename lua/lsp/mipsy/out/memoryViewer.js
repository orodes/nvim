...
exports.setupMemoryButton = void 0;
const fs = require("fs");
const path = require("path");
const vscode = require("vscode");
function setupMemoryButton(context) {
    const allPanels = {};
    const memoryValues = {};
    const selectedBytesPerRow = {};
    function sendPanelData(id) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('mips.debug.viewMemory', () => {
        ...
        const panel = vscode.window.createWebviewPanel('mipsy-memory', 'MIPSY memory', vscode.ViewColumn.Beside, {
            enableScripts: true
        });
        context.subscriptions.push(panel);
        const scriptUri = panel.webview.asWebviewUri(vscode.Uri.joinPath(context.extensionUri, 'out', 'memoryWebviewBundle.js'));
        panel.webview.html = fs.readFileSync(context.asAbsolutePath(path.join('src', 'memoryPanel.html')), { encoding: 'utf-8' }).replace('{{{BUNDLE_URI}}}', scriptUri.toString());
...
exports.setupMemoryButton = setupMemoryButton;
