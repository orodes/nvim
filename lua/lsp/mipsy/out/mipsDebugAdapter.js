...
const scanBuffer_1 = require("./scanBuffer");
const fs = require("node:fs/promises");
// const rand = Math.floor(Math.random() * 9000) + 1000;
const THREAD_ID = 1;
const STEPS_PER_INTERVAL = 300;
class MipsRuntime {
    constructor(source, filename, path, session, scanBuffer) {
        this.source = source;
        ...
        this.isAtExit = false;
        this.runtime = (0, mipsy_vscode_1.make_new_runtime)(source, filename);
    ...
    setAutorun(auto, adapterReason) {
        if (this.autoRunning && !auto) {
            this.session.sendEvent(new debugadapter_1.StoppedEvent(adapterReason, THREAD_ID));
        }
        this.runningReverse = false;
    ...
    step() {
        if (this.isAtExit) {
            ...
            this.session.sendEvent(new debugadapter_1.TerminatedEvent());
            ...
            if (syscallGuard === 'print') {
                ...
                if (!this.scanBuffer.isExhausted) {
                    ...
                    if (syscallGuard === 'read_int') {
                        ...
                        todoSyscall = true;
                        this.session.sendStderrLine(`Queued input with a ${syscallGuard} syscall not currently supported`);
                    }
                    if (result === undefined) {
                        if (this.scanBuffer.isExhausted) {
                            ...
                            this.session.sendStderrLine(`Invalid format encountered in queued input during ${syscallGuard} syscall: ${this.scanBuffer.initialContents()}. Queued input removed.`);
                        }
                ...
                if (this.inputNeeded) {
                    ...
                    this.session.sendStderrLine(`[enter your input to the ${syscallGuard} syscall next to the \`>\` in the box below]`);
                }
                else {
                    this.inputNeeded = true;
                    this.session.sendStdoutLine(`syscall ${syscallGuard}: [enter your input in the box below]`);
                }
    ...
    setBreakpoints(lines) {
        return Array.from(this.runtime.set_breakpoints_from_lines(new Uint32Array(lines)));
    }
    getLineNum() {
    ...
    provideInput(input) {
        ...
        if (result === 'ok') {
            ...
            if (this.resumeOnInput) {
                ...
                this.session.sendEvent(new debugadapter_1.ContinuedEvent(THREAD_ID));
            }
            else {
                // this.session.sendEvent(new InvalidatedEvent(undefined, THREAD_ID));
                this.session.sendEvent(new debugadapter_1.ContinuedEvent(THREAD_ID));
                this.session.sendEvent(new debugadapter_1.StoppedEvent('step', THREAD_ID));
            }
...
class MipsSession extends debugadapter_1.DebugSession {
    constructor() {
        super();
        this.sourceFilePath = '';
        this.source = '';
        this.sourceName = '<source code>';
        this.sourceLines = [];
        this.isVSCode = false;
        this.scanBuffer = new scanBuffer_1.ScanBuffer();
    }
    ...
    sendDebugLine(str) {
        this.sendEvent(new debugadapter_1.OutputEvent(`[debug] ${str}\n`, 'debug console'));
    }
    sendStdoutLine(str) {
        this.sendEvent(new debugadapter_1.OutputEvent(`${str}\n`, 'stdout'));
    }
    sendStderrLine(str) {
        this.sendEvent(new debugadapter_1.OutputEvent(`${str}\n`, 'stderr'));
    }
    ...
    sendSource() {
        ...
        this.sendEvent(new debugadapter_1.LoadedSourceEvent('new', this.getSource()));
    }
    ...
    async launchRequest(response, args, request) {
        const gotSource = () => {
            ...
            this.sourceLines = this.source.split(splitter).map(line => line.replaceAll('\r', '').replaceAll('\n', ''));
            try {
            ...
            this.sendResponse(response);
            this.sendEvent(new debugadapter_1.StoppedEvent('entry', THREAD_ID));
        };
    ...
    pauseRequest(response, args, request) {
        ...
        this.sendEvent(new debugadapter_1.StoppedEvent('pause', THREAD_ID));
    }
    ...
    nextRequest(response, args, request) {
        ...
        this.sendResponse(response);
        this.sendEvent(new debugadapter_1.StoppedEvent('step', THREAD_ID));
    }
  ...
  stackTraceRequest(response, args, request) {
    const lineNum = this.runtime?.getLineNum();
    const safeLine = lineNum && lineNum > 0 ? lineNum : 1;
  ...
  this.sendResponse(response); 
    ...
    disassembleRequest(response, args, request) {
        ...
        response.body = {
            instructions: this.runtime?.disassemble(start, args.instructionCount)?.map((part) => {
                return {
    ...
    setBreakPointsRequest(response, args, request) {
        ...
        let breakpointLines = breakpoints.map(breakpoint => breakpoint.line);
        let linesWithActualBreakpoints = [];
...
//# sourceMappingURL=mipsDebugAdapter.js.map
