...
const lsp_data_json_1 = require("./lsp_data.json");
...
let multiFileDependencies = {};
function getFilenameFromUri(uri) {
    const components = uri.split('/');
    return components[components.length - 1];
}
function setFilenameOfUri(uri, newName) {
    const components = uri.split('/');
...
function getMultifileSources(uri) {
    ...
    allExtraFiles.forEach(extraFile => {
        ...
        if (subDocument !== undefined) {
            ...
            try {
                const path = (0, node_url_1.fileURLToPath)(newUri);
                const source = (0, node_fs_1.readFileSync)(path, { encoding: 'utf-8' });
                result.push({
                    filename: extraFile,
                    source,
                    uri: newUri
...
function getLineWithMultifileDeclaration(uri) {
...
function splitSourceIntoLines(source) {
...
async function validateTextDocument(textDocument) {
    ...
    const diagnostics = [];
    const response = (0, mipsy_vscode_1.test_compile)(source, getFilenameFromUri(textDocument.uri), multiFiles, recheckAmt, false);
    let tabsSize = 8;
    ...
    function correctColumn(lineNum, column) {
        if (column === 0) {
            return column;
        }
        try {
    ...
    for (let err of response.errors) {
        ...
        const diagnostic = {
            severity: err.is_warning ? node_1.DiagnosticSeverity.Warning : node_1.DiagnosticSeverity.Error,
            range: {
                start: {
                    line: lineNum,
...
connection.onCompletion((textDocumentPosition) => {
    ...
    const isStartOfLine = /^[ \t]*$/.test(beforeWithLabelsRemoved.slice(0, beforeWithLabelsRemoved.length - beforeWord.length));
    const isImmeadiateV0 = /^[ \t]*li[ \t]+\$v0[ \t]*,[ \t]*[0-9]{0,2}$/.test(beforeWithLabelsRemoved);
    const allSuggestions = Array.from(lsp_data_json_1.suggestions);
    ...
    allSuggestions.forEach(suggestion => {
        ...
        if (suggestion.type === 'instruction') {
            ...
            item.kind = node_1.CompletionItemKind.Constant;
        }
        else if (suggestion.type === 'label') {
            item.kind = node_1.CompletionItemKind.Reference;
        }
            ...
            item.kind = node_1.CompletionItemKind.Event;
        }
        else {
...
function getWordAtPosition(params) {
...
connection.onDocumentSymbol(params => {
    ...
    definitions.forEach(definition => {
        ...
        const newSymbol = {
            kind: definition.type === 'label' ? node_1.SymbolKind.Class : node_1.SymbolKind.Constant,
    ...
    symbols.reverse();
    function expandRanges(symbols, lastLine) {
...
connection.onHover(params => {
    const word = getWordAtPosition(params);
    const hovers = lsp_data_json_1.hover_docs;
